...
If frmSelf.WindowState = vbMinimized Then
    bSend = True
    frmSrv.Wsk1.SendData Chr$(6) '本地停止发送图象信息 stop sending video
ElseIf frmSelf.WindowState = vbNormal Then
    If bSend Then
        bSend = False
        'ask if continue receiving video
        frmSrv.Wsk1.SendData Chr$(7) '询问远程计算机是否继续接收图象信息
    End If
End If
End Sub
...
On Error Resume Next
frmSrv.Wsk1.SendData Chr$(5) '停止发送图象信息 stop sending video
End Sub
