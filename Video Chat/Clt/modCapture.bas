...
Dim hCapWnd As Long '图像抽取窗口的句柄 handle of video source window
...
Get_SimpleWindow = SendMessage_String(hCapWnd, WM_CAP_FILE_SAVEDIB, 0&, ByVal n) 'Ascii 方式 ASCII method
'Get_SimpleWindow = SendMessage_String(hCapWnd, WM_CAP_FILE_SAVEDIBW, 0&, ByVal StrConv(n, vbUnicode)) 'Unicode 方式 Unicode method
'the following line is for preventing from freezing after captured
Call Set_Preview '加这一行,才不会抽取後,画面呈现冻结(Freeze)状态
End Function
