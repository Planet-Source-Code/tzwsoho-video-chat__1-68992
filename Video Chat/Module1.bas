...
Dim CapHWnd As Long '图像抽取窗口的HWnd handle of video source window
...
B = SendMessage_String(CapHWnd, WM_CAP_FILE_SAVEDIB, 0&, ByVal n) 'Ascii 方式 ASCII method
'B = SendMessage_String(CapHWnd, WM_CAP_FILE_SAVEDIBW, 0&, ByVal StrConv(n, vbUnicode)) 'Unicode 方式 Unicode method
